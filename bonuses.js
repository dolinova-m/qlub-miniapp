import { addDays } from './dates.js?v=3';
import { levelOn } from './status.js?v=3';

// Bonus lessons — «Клубные статусы и правила абонементов.pdf» and user-spec:
// added to every purchased subscription by the status on the purchase day, valid 90 days,
// not tied to a direction, spent oldest first.
...
export function bonusOf(sub, history) {
  const total = BONUS_BY_LEVEL[levelOn(history, sub.bought)];
